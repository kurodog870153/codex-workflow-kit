...
INSTALLER = REPOSITORY_ROOT / "os-scripts" / "mac" / "install-work.command"
...
@unittest.skipUnless(
    sys.platform == "darwin",
    "macOS installer integration tests",
)
class MacWorkInstallerTests(unittest.TestCase):
    def run_installer(
        self,
        user_input: str,
        *,
    ...
    def test_installer_remains_executable(self) -> None:
        self.assertTrue(os.access(INSTALLER, os.X_OK))

    def test_default_home_can_install_only_through_backend(self) -> None:
        with tempfile.TemporaryDirectory(prefix="work-installer-default-") as directory:
            home = Path(directory)
    ...
    def test_custom_home_can_install_jpa_and_mybatis(self) -> None:
        with tempfile.TemporaryDirectory(prefix="work-installer-custom-") as directory:
            home = Path(directory)
            result = self.run_installer(
                f"2\n{home}\n5 6\n",
                user_home=REPOSITORY_ROOT,
            )
    ...
    def test_custom_home_can_install_astro_and_tailwind(self) -> None:
        with tempfile.TemporaryDirectory(prefix="work-installer-frontend-") as directory:
            home = Path(directory)
            result = self.run_installer(
                f"2\n{home}\n9 11\n",
                user_home=REPOSITORY_ROOT,
        ...
        for missing in missing_files:
            for existing in (False, True):
                with self.subTest(missing=missing, existing=existing), tempfile.TemporaryDirectory(
                    ...
                    result = self.run_installer(
                        "1\n3\n", user_home=home, installer=installer
                    )
