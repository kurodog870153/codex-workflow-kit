...
from worklib.models.invocation import InvocationContract as LegacyInvocationContract
...
class InvocationTests(unittest.TestCase):
    ...
    def test_modes_preserve_opaque_request_exactly(self):
        ...
        for mode in ("plan", "task", "execute"):
            ...
            self.assertEqual(result, {"schema": "work-invocation/v1", "mode": mode, "request": tail, "entry": {"kind": "workflow"}})
    ...
    def test_rejects_missing_fields_extra_headers_and_private_modes(self):
        cases = {"請使用 $work plan -- example": "work_invocation_not_explicit", "`$work plan -- example`": "work_invocation_not_explicit", "$workflow plan -- example": "work_invocation_not_explicit", "$work": "work_invocation_mode_missing", "$work -- example": "work_invocation_mode_missing", "$work Plan -- example": "work_invocation_mode_invalid", "$work artifact-editor -- example": "work_invocation_mode_invalid", "$work progress-saver -- example": "work_invocation_mode_invalid", "$work task web/backend -- example": "work_invocation_delimiter", "$work plan --request": "work_invocation_delimiter", "$work execute": "work_invocation_delimiter", "$work plan -- \n\t": "work_invocation_request_missing"}
    ...
    def test_explicit_resume_and_task_planning_routes(self):
        ...
        self.assertEqual(self.parse("$work task -- example-12")["entry"], {"kind": "task_planning", "requirement_id": "example-12"})
