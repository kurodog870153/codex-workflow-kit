...
from worklib.models.delegation import (
    DelegationBuildRequestContract, DelegationEnvelopeContract, DelegationValidationContract,
)
from worklib.models.delegation import (
...
class DelegationContractTests(unittest.TestCase):
    ...
    def test_examples_validate_and_use_canonical_order(self) -> None:
        for contract in (DelegationBuildRequestContract, DelegationEnvelopeContract, DelegationValidationContract):
            with self.subTest(contract=contract.contract_id):
                model = contract.model_validate(contract.contract_example)
                self.assertEqual(list(model.to_canonical_dict()),
                                 [key for key in contract.canonical_order if key in contract.contract_example])
