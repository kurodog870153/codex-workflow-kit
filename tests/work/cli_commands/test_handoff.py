...
from worklib.workflows.handoff import HandoffOperations
...
class HandoffCliTests(FileInputTestCase):
    def test_return_verifiers_dispatch_independent_targets_and_context(self):
        for direction, context in (("task-to-plan", []), ("execute-to-task", ["--preflight"]),
            ...
            with tempfile.TemporaryDirectory() as temporary:
                ...
                output = io.StringIO()
                with patch("worklib.business_services.handoff.verify_return_handoff", return_value={"status": "valid"}) as verify:
    ...
    def test_verify_task_dispatches_confirmed_target_and_roots(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("worklib.business_services.handoff.verify_task_to_execute_handoff", return_value={"status": "valid"}) as verify:
    ...
    def test_verify_plan_dispatches_confirmed_path_and_roots(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            incoming = {"marker": "WORK-HANDOFF"}
            skill_root = "repo:skills=" + str(root / "skills")
            with patch("worklib.business_services.handoff.verify_plan_to_task_handoff", return_value={"status": "valid"}) as verify:
    ...
    def test_preflight_return_dispatches_target_and_skill_roots(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for target in ("task", "plan"):
                output, error = io.StringIO(), io.StringIO()
                with patch("worklib.business_services.handoff.build_preflight_return_handoff", return_value={"marker": "WORK-HANDOFF"}) as build:
                    code = main(self.input_arguments([
    ...
    def test_build_task_to_plan_dispatches_optional_target_and_roots(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for task_id in (None, "TASK-002"):
                ...
                with patch("worklib.business_services.handoff.build_task_to_plan_handoff", return_value={"marker": "WORK-HANDOFF"}) as build:
                    code = main(self.input_arguments([
    ...
    def test_build_task_to_execute_dispatches_explicit_target(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            skill_root = "repo:skills=" + str(root / "skills")
            with patch("worklib.business_services.handoff.build_task_to_execute_handoff", return_value={"marker": "WORK-HANDOFF"}) as build:
                code = main(self.input_arguments([
    ...
    def test_build_plan_to_task_dispatches_semantic_input_and_skill_roots(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            skill_root = "repo:skills=" + str(root / "skills")
            with patch("worklib.business_services.handoff.build_plan_to_task_handoff", return_value={"marker": "WORK-HANDOFF"}) as build:
                code = main(self.input_arguments([
            ...
            self.assertEqual((code, error.getvalue()), (ExitCode.SUCCESS, ""))
            self.assertEqual(json.loads(output.getvalue())["data"], {"marker": "WORK-HANDOFF"})
