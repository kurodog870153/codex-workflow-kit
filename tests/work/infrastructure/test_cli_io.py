...
class CliFileTransportTests(FileInputTestCase):
    ...
    def test_non_regular_or_missing_file_never_dispatches_or_acquires_lock(self):
        ...
        for path in (self.root / "missing.json", directory):
            with self.subTest(path=path), patch("worklib.cli.run_execute") as operation:
                response = self.invoke("execute", "record-finish", "--input-file", str(path), "--user-config-root", str(self.root), "--task-path", "outputs/work/tasks/example/task.json", "--execution-dir", "execution", "--task-id", "TASK-001", expected_code=ExitCode.IO_FAILURE)
    ...
    def test_removed_stdin_returns_replacement_hint_without_reading(self):
        for option in ("--stdin", "--stdin=true"):
            with self.subTest(option=option), patch("sys.stdin") as legacy_input:
                response = self.invoke("task", "draft-init", option, expected_code=ExitCode.CLI_USAGE)
    ...
    def test_missing_input_and_conflicting_sources_are_usage_errors(self):
        for arguments in (("attempt", "render"), ("attempt", "render", "--input-file"), ("attempt", "validate", "--path", "formal.json", "--input-file", "request.json")):
    ...
    def test_unexpected_exception_is_structured_without_exception_contents(self):
        with patch("worklib.cli._run", side_effect=RuntimeError("private exception contents")):
            response = self.invoke("paths", "resolve", "--requirement-id", "example", expected_code=ExitCode.INTERNAL_ERROR)
    ...
    def test_every_command_help_is_one_json_response_without_required_inputs(self):
        ...
        for command in commands(build_parser()):
            ...
            self.assertIn("usage:", json.loads(stdout.getvalue())["data"]["help"])
    ...
    def test_real_process_uses_cwd_request_paths_and_utf8_without_shell(self):
        ...
        command = [sys.executable, "-B", str(SCRIPT_ROOT / "work.py"), "--project-root", str(self.root), "hierarchy", "selection-build", "--input-file", path.name]
        for failure in (False, True):
            arguments = command if not failure else command[:-1] + ["找不到 missing.json"]
            result = subprocess.run(arguments, cwd=path.parent, stdin=subprocess.DEVNULL, capture_output=True, shell=False, timeout=30, env={**os.environ, "PYTHONIOENCODING": "ascii", "PYTHONUTF8": "0"})
    ...
    def test_renderer_data_preserves_canonical_order_and_round_trips(self):
        ...
        handoff = {
            ...
            "target": {"stage": "task"}, "summary": "繼續討論。",
        ...
        for command, contract, canonical in cases:
            response = self.invoke(command, "render", "--input-file", self.input_file(json.dumps(contract)))
