...
from ...models.execution.correction import CorrectionCreateContract
...
from ...services.execution.correction import read_raw, parse_json_contract, resolve_project_relative_path
from ...services.skill_catalog import SkillRoot
from ...services.correction.impact import affected_task_ids
from ...services.correction.lock import build_correction_lock as _build_lock
from ...services.correction.state import corrected_index
from .correction_request import parse_correction_create_request
from ...services.execution.correction import (
    consume_temporary,
    install_exclusive,
    prepare_file,
    replace_file,
...
def _correction_affected_task_ids(
    index: dict[str, Any],
    ...
    *,
    ...
    return affected_task_ids(index, task_contract, task_id=task_id, invalidates_completion=invalidates_completion)


def _build_corrected_index(
    index: dict[str, Any],
    *,
    task_id: str,
    correction_id: str,
    ...
    statuses = ["pending_retry" if row["id"] in affected else row["status"] for row in index["tasks"]]
    return corrected_index(index, task_id=task_id, correction_id=correction_id, affected_task_ids=affected_task_ids, overall_status=derive_overall_status(statuses))
...
def create_correction(
    ...
    *,
    ...
    affected_task_ids = _correction_affected_task_ids(
        index,
    ...
    final_index = _build_corrected_index(
        locked_index,
        task_id=task_id,
        correction_id=correction_id,
        affected_task_ids=affected_task_ids,
    ...
    validate_correction_file(project_root, correction_relative, validate_attempt_file=validate_attempt_file)
...
def recover_correction(
    ...
    *,
    ...
    expected_affected = _correction_affected_task_ids(
        base_index,
        context["task_contract"],
        task_id=task_id,
