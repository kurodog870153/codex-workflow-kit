...
from ...models.execution.attempt_start import (
...
from ...services.execution.lifecycle import read_raw, parse_json_contract, resolve_project_relative_path, validate_execution_task_layout
from ...services.skill_catalog import SkillRoot
from ...services.execution.lifecycle import (
    replace_index as _replace_index_bytes,
...
def _replace_index(
    *,
    ...
    stored = _replace_index_bytes(
        index_path=index_path,
        expected_current=expected_current,
        rendered=rendered,
        temporary_path=temporary_path,
        allow_existing_temporary=allow_existing_temporary,
    )
...
def _locked_index(
    ...
    return _locked_index_state(index, lock=lock)
...
def _started_index(
    ...
    statuses = [
        "in_progress" if item["id"] == task_id else item["status"]
