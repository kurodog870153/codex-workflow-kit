...
from ...services.task.draft.storage import _decode, _error, _path, _read, _render, _write, read_task_draft_from_index, read_task_planning_index, read_task_planning_revision
from .plan import validate_task_source_plan as validate_plan_contract
from ...services.task.draft.validation import validate_task_draft, validate_task_planning_index
...
from ...services.task.draft.validation import validate_draft_instruction_selection
...
def update_task_draft_sources(
    ...
    recover: bool = False, operations=None,
) -> dict[str, object]:
...
"""
    ...
    validation = operations.validate_plan_contract(raw_plan, source=str(resolved), actual_plan_path=normalized,
    ...
    for entry in previous["tasks"]:
        ...
        instruction = operations.build_instruction_selection(skill_root=work_root, mode="task", selected_paths=selected["selected_paths"], reference_names=selected["references"])
