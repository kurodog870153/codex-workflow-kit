...
class CommandCorrectionContract(WorkContract):
    ...
    canonical_order: ClassVar[tuple[str, ...]] = ('schema', 'task_id', 'attempt_id', 'record_id', 'index_path', 'correction_status', 'lock_status')
...
class DirectCommandInvocationModel(CommandInvocationModel):
    kind: Literal['direct']
    executable: str
    executable_sha256: Annotated[str, Field(pattern=SHA256_PATTERN)]
    argv: list[str]
...
class WindowsBatchInvocationModel(CommandInvocationModel):
    ...
    launcher: str
    launcher_sha256: Annotated[str, Field(pattern=SHA256_PATTERN)]
    script: str
    script_sha256: Annotated[str, Field(pattern=SHA256_PATTERN)]
    arguments: list[str]
    command_line: str
...
class CommandStartedContract(WorkContract):
    ...
    canonical_order: ClassVar[tuple[str, ...]] = ('schema', 'preview', 'authorization_evidence')
...
class CommandResultContract(WorkContract):
    ...
    contract_kind: ClassVar[Literal['response']] = 'response'
    canonical_order: ClassVar[tuple[str, ...]] = ('schema', 'approved_sha256', 'record_id', 'status', 'exit_code', 'stdout_tail', 'stdout_truncated', 'stderr_tail', 'stderr_truncated', 'receipt_prefix', 'record_finish_required', 'record_finish_request')
