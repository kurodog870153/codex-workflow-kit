...
class SpecificationExpectedModel(BaseModel):
    ...
    task_index_sha256: Annotated[str, Field(pattern=r"^[0-9a-f]{64}$")]
    execution_index_sha256: Annotated[str, Field(pattern=r"^[0-9a-f]{64}$")]
    task_item_sha256: dict[str, Annotated[str, Field(pattern=r"^[0-9a-f]{64}$")]]
...
class SpecificationVerificationContract(WorkContract):
    ...
    requirement_id: str
    artifacts: TaskArtifactsModel
    task_collection_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
