...
_nonempty_string = ContractValuePolicy.nonempty_string
_sha256 = ContractValuePolicy.sha256
_strict_keys = ContractValuePolicy.strict_keys
...
def validate_task_context_shape(
    contract: dict[str, Any],
    *,
    top_required: set[str],
    top_optional: set[str],
    task_required: set[str],
    task_optional: set[str],
    historical_work_sources: bool,
    source_plan_raw: bytes | None,
    reviewed_source_plan_binding: tuple[str, str] | None,
) -> list[dict[str, Any]]:
    if reviewed_source_plan_binding is not None and (
        not historical_work_sources or source_plan_raw is None
    ):
        raise WorkError(
            ExitCode.CONTRACT,
            "invalid_source_plan_repair_context",
...
def validate_task_context_plan(
    contract: dict[str, Any],
    ...
    *,
    ...
    if actual_plan_sha != source_plan_sha and reviewed_source_plan_binding != (
        source_plan_sha,
        actual_plan_sha,
    ):
        raise WorkError(
    ...
    task_ids: list[str] = []
    for task in tasks:
        task_id = _nonempty_string(task["id"], location="task.id")
    ...
    for task in tasks:
        ...
        if any(dependency not in task_id_set or dependency == task_id for dependency in direct):
            raise WorkError(
                ExitCode.CONTRACT,
...
def _validate_task_contract_object(
    ...
    *,
    ...
    if _reviewed_source_plan_binding is not None and (
        not context["historical_work_sources"] or context["plan_raw"] is None
    ):
        raise WorkError(
            ExitCode.CONTRACT, "invalid_source_plan_repair_context",
            "Reviewed binding repair requires an explicit historical source Plan.",
        )
    _strict_keys(contract, location="task", required=context["top_required"], optional=context["top_optional"])
    if contract["schema"] != "work-task-collection-projection/v1" or contract["status"] != "confirmed":
    ...
    plan_raw = context["plan_raw"]
    plan_validation = context["plan_validation"]
    if (
    ...
    for index, raw_task in enumerate(raw_tasks):
        task = _strict_keys(
            raw_task,
            location=f"tasks[{index}]",
            required=context["task_required"],
            optional=context["task_optional"],
        )
    ...
    if validate_file_state:
        existence = dict(context["path_existence"])
    ...
    if "changes" in contract and not context["changes_validated"]:
        raise WorkError(ExitCode.CONTRACT, "invalid_task_changes_context", "TASK changes were not validated.")
...
def validate_task_contract(
    ...
    *,
    ...
    try:
        return _validate_task_contract(
            raw,
            source=source,
...
def _validate_task_contract(
    ...
    *,
    ...
    ordered = context["ordered_contract"]
    require_canonical_json_contract(
        raw,
        contract=ordered,
        source=source,
