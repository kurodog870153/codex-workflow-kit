...
def _error(exit_code: ExitCode, code: str, message: str, **details: object) -> None:
...
def validate_execution_identity(
    *, task_contract: dict[str, Any], task_validation: dict[str, object],
    index: dict[str, Any], attempt: dict[str, Any], task_id: str,
) -> dict[str, Any]:
    source_fingerprints = {
    ...
    if observed_index != expected_index:
        _error(ExitCode.ARTIFACT_INTEGRITY, "record_begin_index_identity_mismatch", "The execution index does not match the formal TASK identity.", expected=expected_index, actual=observed_index)
    expected_ids = task_validation["task_ids"]
    if [item["id"] for item in index["tasks"]] != expected_ids:
        _error(ExitCode.ARTIFACT_INTEGRITY, "record_begin_index_task_set_mismatch", "The execution index TASK set does not match the formal TASK.")
    ...
    if row["instructions_sha256"] != task_instructions[task_id]:
        _error(ExitCode.ARTIFACT_INTEGRITY, "record_begin_task_instructions_mismatch", "The target TASK instruction fingerprint is stale.")
    expected_attempt = {
    ...
    if observed_attempt != expected_attempt:
        _error(ExitCode.ARTIFACT_INTEGRITY, "record_begin_attempt_identity_mismatch", "The active Attempt does not match the formal TASK identity.", expected=expected_attempt, actual=observed_attempt)
    return row
