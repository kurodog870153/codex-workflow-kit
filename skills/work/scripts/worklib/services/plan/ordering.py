...
def _order_hierarchy_selection(value: object) -> object:
    if not isinstance(value, dict):
        return value
    ordered = _ordered_object(
        value,
        ("schema", "decision", "selected_paths", "entries", "catalog_sha256", "selection_sha256"),
    )
    assert isinstance(ordered, dict)
    if isinstance(ordered.get("entries"), list):
        entries: list[object] = []
        for raw_entry in ordered["entries"]:
            entry = _ordered_object(
                raw_entry,
                ("path", "mode_support", "mode_metadata", "recommendation_reason"),
            )
            if isinstance(entry, dict) and isinstance(entry.get("mode_metadata"), dict):
                raw_metadata = entry["mode_metadata"]
                metadata_by_mode: dict[str, object] = {}
                for mode in ("plan", "task", "execute"):
                    if mode in raw_metadata:
                        metadata_by_mode[mode] = _ordered_object(
                            raw_metadata[mode], ("name", "description", "work_tags")
                        )
                for mode in sorted(set(raw_metadata) - set(metadata_by_mode)):
                    metadata_by_mode[mode] = raw_metadata[mode]
...
def order_plan_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if "hierarchy_selection" in ordered:
        ordered["hierarchy_selection"] = _order_hierarchy_selection(
