...
def parse_work_instruction_selection(value: object, *, selected_paths: list[str], location: str = "work_instruction_selection") -> dict[str, object]:
    ...
    for index, source in enumerate(raw_sources):
        ...
        if not isinstance(source, dict) or set(source) not in (SOURCE_FIELDS, LEGACY_SOURCE_FIELDS):
            ...
            missing = sorted(expected - set(source)) if isinstance(source, dict) else sorted(expected)
            unknown = sorted(set(source) - expected) if isinstance(source, dict) else []
    ...
    result = {"selected_paths": stored_paths, "resolved_paths": resolved_paths, "sources": sources, "references": references, "instructions_sha256": digest}
    if "routing_manifest" in value:
        result["routing_manifest"] = value["routing_manifest"]
    return result
