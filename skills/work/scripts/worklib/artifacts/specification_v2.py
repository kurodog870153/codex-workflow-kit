"""Prepare, publish, recover and verify v2 TASK collection revisions."""
...
from ..contracts.execution_index import build_initial_execution_index, derive_overall_status, render_execution_index, validate_execution_index
...
def _artifacts(plan: dict[str, Any], plan_path: str) -> dict[str, str]:
    ...
    if artifacts["plan"] != plan_path or not str(artifacts["task"]).endswith("/index.json"):
        _fail("spec_v2_artifact_identity", "The Plan must route this revision to a v2 TASK index.")
    return artifacts
...
def _pointer(value: dict[str, Any], pointer: str, *, operation: str, before: object = None, after: object = None) -> None:
    if pointer == "/":
        _fail("spec_v2_root_edit", "Whole-document edits are handled by TASK item add/remove operations.")
    if not pointer.startswith("/") or "/" in pointer[1:]:
        _fail("spec_v2_pointer", "Preparation supports one top-level JSON Pointer per edit.", path=pointer)
    key = pointer[1:].replace("~1", "/").replace("~0", "~")
    present = key in value
    if operation == "add":
        if present:
            _fail("spec_v2_edit_state", "An add target already exists.", path=pointer)
        value[key] = copy.deepcopy(after)
    elif operation == "replace":
        if not present or value[key] != before:
            _fail("spec_v2_edit_state", "A replace target does not match reviewed before evidence.", path=pointer)
        value[key] = copy.deepcopy(after)
    elif operation == "remove":
        if not present or value[key] != before:
            _fail("spec_v2_edit_state", "A remove target does not match reviewed before evidence.", path=pointer)
        del value[key]
    else:
        _fail("spec_v2_edit_operation", "The edit operation is invalid.")
...
def _execution(old: dict[str, Any], logical: dict[str, Any], validation: dict[str, Any], affected: list[str], change_id: str) -> dict[str, Any]:
    ...
    for fresh in generated["tasks"]:
        ...
        if row["id"] in affected and row["status"] != "pending":
            if row["status"] == "cancelled":
                _fail("spec_update_cancelled_task", "A cancelled TASK requires an explicit lifecycle decision.")
...
def prepare_v2(raw_request: bytes, *, project_root: Path, user_config_root: str, skill_roots=None, output_file: str | None = None) -> dict[str, object]:
    request = strict_keys(parse_json_contract(raw_request, source="v2 specification preparation"), location="spec_prepare",
                          required={"schema", "plan_path", "reason", "edits"})
    if request["schema"] != "work-spec-prepare-request/v1":
        _fail("spec_prepare_schema", "Use work-spec-prepare-request/v1.")
    ...
    if not isinstance(edits, list) or not edits:
        _fail("spec_prepare_edits", "Supply non-empty v2 edits.")
    seen: set[tuple[object, ...]] = set()
    normalized: list[dict[str, Any]] = []
    plan_changed = False
    for position, raw_edit in enumerate(edits):
        edit = strict_keys(raw_edit, location=f"edits[{position}]", required={"artifact", "operation", "path"}, optional={"task_id", "before", "after", "affected_ids"})
        artifact, operation, pointer = edit["artifact"], edit["operation"], edit["path"]
        identity = (artifact, edit.get("task_id"), pointer)
        if identity in seen:
            _fail("spec_prepare_duplicate", "Each v2 target may be edited once.")
        seen.add(identity)
        required = {"add": {"after"}, "replace": {"before", "after"}, "remove": {"before"}}
        if operation not in required or not required[operation] <= set(edit):
            _fail("spec_v2_edit_fields", "Edit evidence does not match its operation.")
        if artifact == "plan":
            if "task_id" in edit or "affected_ids" not in edit:
                _fail("spec_v2_plan_edit", "Plan edits require affected_ids and reject task_id.")
            ...
            if "task_id" in edit:
                _fail("spec_v2_index_edit", "TASK index edits reject task_id.")
            ...
            if pointer == "/" and operation == "add":
                if task_id in items or not isinstance(edit.get("after"), dict):
                    _fail("spec_v2_item_add", "A new TASK item requires a new ID and complete object.")
                ...
                if task_id not in items or edit.get("before") != items[task_id]:
                    _fail("spec_v2_item_remove", "Removed TASK item evidence does not match.")
                del items[task_id]
            else:
    ...
    execution = _execution(baseline["execution"], validation["logical_contract"], validation, affected, change_id)
    execution_raw = render_execution_index(execution)
    validate_execution_index(execution_raw, source="candidate execution index")
    prepared = {"schema": "work-spec-update-request/v2", "reason": request["reason"],
    ...
    preview = validate_v2(prepared, baseline=baseline, project_root=project_root, user_config_root=user_config_root, skill_roots=skill_roots)
    ...
    return {"schema": "work-spec-prepare/v2", "request": prepared, "preview": preview, "output_file": output_file,
...
def _candidate_sources(request: dict[str, Any], baseline: dict[str, Any], *, project_root: Path, user_config_root: str, skill_roots=None):
    ...
    execution = _execution(baseline["execution"], validation["logical_contract"], validation, affected, request["task_index"]["changes"][-1]["id"])
    return plan_raw, index_raw, item_raw, render_execution_index(execution), validation, affected


def validate_v2(request: dict[str, Any], *, baseline: dict[str, Any], project_root: Path, user_config_root: str, skill_roots=None) -> dict[str, object]:
    strict_keys(request, location="spec_update_v2", required={"schema", "reason", "expected", "plan", "task_index", "task_items"})
    if request["schema"] != "work-spec-update-request/v2":
        _fail("spec_update_schema", "Use work-spec-update-request/v2.")
    ...
    if expected != observed:
        _fail("spec_update_source_changed", "The reviewed v2 source fingerprints changed.")
    ...
    metadata = {"request": request, "artifacts": baseline["artifacts"], "affected_task_ids": affected,
                "history_sha256": _history(project_root, baseline["artifacts"]["execution"]),
    ...
    journal = {"schema": "work-spec-transaction/v2", "transaction_id": transaction_id,
    ...
    changed_fields = _changed_fields_v2(
        baseline["plan"], request["plan"], baseline["index"], request["task_index"],
        baseline["items"], request["task_items"],
    )
    return {"schema": "work-spec-update/v2", "status": "valid", "requirement_id": request["plan"]["requirement_id"],
            ...
            "transaction": journal, "file_readiness": "requires_execute_preflight"}


def update_v2(raw_request: bytes, *, project_root: Path, user_config_root: str, skill_roots=None, operation="validate", approved_sha256=None) -> dict[str, object]:
    request = parse_json_contract(raw_request, source="v2 specification update")
    plan = request.get("plan")
    if not isinstance(plan, dict): _fail("spec_update_candidate", "A complete Plan is required.")
    ...
    if operation == "recover":
        ...
        result = {"schema": "work-spec-update/v2", "status": "valid", "requirement_id": plan["requirement_id"], "record_id": journal["transaction_id"],
        ...
        result = validate_v2(request, baseline=baseline, project_root=project_root, user_config_root=user_config_root, skill_roots=skill_roots)
    ...
    if approved_sha256 != result["approved_sha256"]: _fail("spec_update_approval_changed", "The approved v2 transaction changed.")
    from .specification import _history, _publication_followup
    if _history(project_root, artifacts["execution"]) != journal["metadata"]["history_sha256"]: _fail("spec_update_history_changed", "Execution history changed before publication.")
    marker_relative = journal_relative + ".done"
    with state_writer(project_root, artifacts["execution"]):
        if _history(project_root, artifacts["execution"]) != journal["metadata"]["history_sha256"]:
        ...
        try:
            ...
            raise WorkError(
                ...
                "Preserve the v2 specification transaction and obtain recovery authorization.",
    ...
    for path, expected_sha in journal["metadata"]["candidate_sha256"].items():
        if raw_sha256(read_raw(storage_path(project_root, path))) != expected_sha:
            _fail("spec_update_post_write", "An installed v2 artifact differs from approval.", path=path)
    ...
    _publication_followup(result, migration=False)
    return result


def verify_v2(raw_request: bytes, *, project_root: Path, user_config_root: str, skill_roots=None) -> dict[str, object]:
    request = parse_json_contract(raw_request, source="v2 specification verification")
    ...
    if not completion_marker_matches(raw, marker): _fail("spec_verify_completion_mismatch", "The v2 completion marker does not match.")
    for path, expected in journal["metadata"]["candidate_sha256"].items():
        if raw_sha256(read_raw(storage_path(project_root, path))) != expected: _fail("spec_verify_state_changed", "A v2 candidate file changed.", path=path)
    validation = _load(project_root, user_config_root, artifacts["plan"], skill_roots=skill_roots)["validation"]
    return {"schema": "work-spec-verification/v2", "status": "verified", "verified": True,
            ...
            "execution_authorized": False, "next_step": "normal_execute_preflight"}
