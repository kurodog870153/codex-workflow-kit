...
9. [強制] Attempt-start 中斷以固定 `.work-attempt-start-<TASK-ID>-<ATTEMPT-ID>-<stage>.tmp`、index lock 與 Attempt 是否存在判定階段；一般 preflight 發現暫存檔時停止。只有重新取得授權後，才能以原路徑、input IDs 及 `work-attempt-start-request/v1` 執行 `execute recover-attempt-start --input-file "<request-path>"`。工具只安裝內容完全相符的預備 index、補建同一 canonical Attempt 或同步同一 index；任何衝突均保留現況並停止。
