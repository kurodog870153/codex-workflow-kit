...
2. Require the user to identify one formal TASK entry point and one `TASK-*` from that specification before eligibility checks. Do not select either on the user's behalf. For v2, the loader reads the selected item and only the dependency data required by the operation; full collection validation remains available when required.
3. Use the default TASK path `outputs/work/tasks/<requirement-id>/index.json` unless the user explicitly supplies and confirms a permitted project-relative path. A retained v1 `task.json` may be used only when selected by its validated Plan.
...
2. After explicit start authorization, save pure `work-attempt-start-request/v1` JSON containing `worktree_snapshot_sha256` as the request file and invoke `<work-cli> execute attempt-start --input-file "<request-path>"` with the same preflight arguments. For `pending_retry`, also provide the latest source Attempt and only carried record IDs whose current evidence was explicitly confirmed.
...
2. Review the exact argv, selected executable, working directory, timeout, source fingerprints, expected effects and receipt paths. Obtain explicit command execution authorization and required platform/sandbox permission for those exact effects. A fingerprint or text assertion is not permission. Never invoke this helper to bypass a tool restriction or hide a command that requires platform approval. Existing worktree, input, skill-loading and side-effect review still apply; the preview does not attest to executable dependencies, script arguments' file contents, environment values or remote state.
3. Run `<work-cli> execute command-run --input-file "<same-request-path>" --approved-sha256 "<preview-fingerprint>" --authorization-evidence "<concise actual approval evidence>"`. It rechecks the preview under the shared writer mutex, exclusively writes an Attempt-local `.work-command-<record-instance>.started.json` receipt, launches the selected executable once with `shell=False`, no stdin and unchanged remaining argv, then exclusively writes `.finished.json`. Retry IDs use `-retry-n` in receipt filenames. Shell expansion, automatic correction, retries and other CMDs are not performed. Shell-mode commands and `.bat`/`.cmd` executables are rejected. The declared shell is context metadata, never implicitly launched. Executable symlink names are preserved for virtual environment behavior.
...
1. After the reserved CMD, OP, or VAL finishes, save pure `work-record-finish-request/v1` JSON as the request file and invoke `<work-cli> execute record-finish --input-file "<request-path>"` with the same TASK and execution paths. `record.id` must exactly match the reserved lock ID, including any retry suffix.
...
1. After all authorized records finish or execution must stop, obtain specific close authorization and save pure `work-attempt-close-request/v1` JSON as the request file and invoke `<work-cli> execute attempt-close --input-file "<request-path>"` with the same TASK and execution paths.
