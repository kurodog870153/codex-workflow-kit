...
2. After separate authorization, save pure `work-correction-create-request/v1` JSON as the request file and invoke `<work-cli> execute correction-create --input-file "<request-path>"`. Provide `target_attempt_id`, `field`, `correct_value`, `reason`, and explicit boolean `invalidates_completion`. The target Attempt is a semantic choice because a Correction may address an older closed Attempt; do not infer it from the latest Attempt.
